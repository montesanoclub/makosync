...
package-imports by routing through ``dolphinsync.__main__:main``.

Run by PyInstaller in onefile mode as a top-level script, so relative imports
inside dolphinsync.__main__ would otherwise fail with "no known parent
...
from dolphinsync.__main__ import main
